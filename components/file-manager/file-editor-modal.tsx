...
export function FileEditorModal({
  ...
  const extensionBadge = useMemo(() => {
    ...
    if (extension === 'json') return { label: 'JSON', icon: <FileJson className={commonClass} /> };
    if (['ts', 'tsx', 'js', 'jsx', 'mjs', 'cjs'].includes(extension))
      return { label: extension.toUpperCase(), icon: <FileCode className={commonClass} /> };
    ...
    if (['css', 'scss', 'less'].includes(extension))
      return { label: extension.toUpperCase(), icon: <FileCode className={commonClass} /> };
    if (['py', 'rb', 'php', 'go', 'rs', 'java', 'c', 'cpp', 'h', 'hpp'].includes(extension))
      return { label: extension.toUpperCase(), icon: <FileCode className={commonClass} /> };
    if (['sql', 'csv'].includes(extension))
      return { label: extension.toUpperCase(), icon: <FileSpreadsheet className={commonClass} /> };
    if (['env', 'ini', 'conf', 'config', 'toml'].includes(extension))
      return { label: extension.toUpperCase(), icon: <FileType className={commonClass} /> };
    if (extension === 'docker') return { label: 'DOCKER', icon: <Container className={commonClass} /> };
    if (extension === 'sh' || extension === 'bash' || extension === 'zsh')
      return { label: 'SHELL', icon: <TerminalSquare className={commonClass} /> };
    return { label: extension ? extension.toUpperCase() : 'TXT', icon: <FileText className={commonClass} /> };
  }, [extension]);
      ...
      >
        ...
        <div className="flex flex-col h-full min-h-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex flex-col gap-1">
                  <ModalTitle className={`${text.headingLarge} text-white drop-shadow`}>
                    {fileName || 'Untitled'}
                  </ModalTitle>
          ...
          <div className="p-6 space-y-3 bg-white/5 h-full min-h-0 flex flex-col">
            ...
            <div className={`${card.base} flex-1 min-h-0 overflow-hidden border-white/10`}>
              <MonacoEditor
                height="100%"
                language={language}
          ...
          <div className="px-6 py-4 flex items-center justify-between border-t border-white/10 bg-black/30 backdrop-blur">
            ...
            <div className={`${text.muted} text-[11px] break-all text-right`}>{path}</div>
          </div>
        </div>
      </ModalContent>
    </Modal>
